...
def run_ui(calculate_callback: Optional[Callable[[List[float], List[float], float, float, float], dict]] = None):
    # --- Hilfsfunktionen ---
    def _parse_float_list(entry: tk.Entry, description: str) -> List[float]:
        raw_values = [value.strip() for value in entry.get().split(',') if value.strip()]
        try:
            return [float(value) for value in raw_values]
        except ValueError as exc:
            raise ValueError(f"{description} müssen numerisch sein.") from exc

    def _validate_positive(values: Sequence[float], description: str) -> None:
        if any(value <= 0 for value in values):
            raise ValueError(f"Alle {description} müssen > 0 sein.")

    def _parse_inputs() -> CalculationInput:
        try:
            n_layers = int(entry_layers.get())
        except ValueError as exc:
            raise ValueError("Schichtanzahl muss eine ganze Zahl sein.") from exc
        if n_layers <= 0:
            raise ValueError("Schichtanzahl muss > 0 sein.")

        thicknesses = _parse_float_list(entry_thickness, "Dicken")
        ks = _parse_float_list(entry_k, "Wärmeleitwerte")

        if len(thicknesses) != n_layers or len(ks) != n_layers:
            raise ValueError("Anzahl der Werte muss der Schichtanzahl entsprechen.")

        _validate_positive(thicknesses, "Dicken")
        _validate_positive(ks, "Wärmeleitwerte")
        ...
        try:
            ...
            raise ValueError("Temperaturen und h müssen numerisch sein.") from exc

        if h_value <= 0:
            raise ValueError("Wärmeübergangskoeffizient h muss > 0 sein.")

        return CalculationInput(thicknesses, ks, T_left, T_inf, h_value)

    def _calculate_with_callback(inputs: CalculationInput):
        callback = calculate_callback or compute_multilayer
        return callback(inputs.thicknesses, inputs.ks, inputs.T_left, inputs.T_inf, inputs.h)

    def calculate():
        try:
            inputs = _parse_inputs()
            result = _calculate_with_callback(inputs)
    ...
    def save_current_project(name_entry: tk.Entry):
        ...
        try:
            inputs = _parse_inputs()
            result = _calculate_with_callback(inputs)
            if save_project(name, inputs.thicknesses, inputs.ks, inputs.T_left, inputs.T_inf, inputs.h, result):
